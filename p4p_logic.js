(function(){
  // P4P: one row per fighter name, persistent manual P4P position, aggregated record across divisions.
  window.p4p=function(){
    const map=new Map();
    F.forEach(f=>{
      const key=f.name.trim().toLowerCase();
      if(!map.has(key)) map.set(key,{...f,wins:0,losses:0,draws:0,titleWins:0,titleDefenses:0,activeDefenses:0,streak:0,weights:[],p4pRank:null,champion:false});
      const x=map.get(key);
      ...
      if(f.p4pRank && (!x.p4pRank || Number(f.p4pRank)<Number(x.p4pRank)))x.p4pRank=Number(f.p4pRank);
  ...
  window.rank=function(m){
    if(weight==='P4P'){
      ...
      m.innerHTML=`<div class="card"><div class="muted">RANKING</div><div class="hero" style="font-size:25px">P4P</div><div class="muted">Единый рекорд бойца из всех весовых. Один боец — одна строка. Позиция P4P сохраняется отдельно.</div></div><div class="list">${top.map((f,i)=>row(f,i+1)).join('')}</div>${out.length?`<div class="outside"><div class="outsideTitle">За пределами TOP-15</div><div class="list">${out.map((f,i)=>row(f,i+16)).join('')}</div></div>`:''}`;
      return;
    }
  ...
  // Never mutate F while opening P4P. Stored champion and ranking data are authoritative.
